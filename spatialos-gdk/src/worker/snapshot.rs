use worker::{ffi, Connection, EntityId, ComponentId};
use std::{mem, ptr};
...
impl SnapshotOutputStream {
	...
	pub fn write_entity(&mut self, components: HashMap<ComponentId, Box<ffi::Schema_ComponentData>>,
        entity_id: EntityId) -> Result<(), &str> {
		unsafe {
