...
impl Module {
    ...
    pub fn add_tokens(&mut self, mut remaining_modules: Vec<String>, tokens: Tokens) {
        if remaining_modules.len() == 0 {
            ...
            let next_module = self.sub_modules
                .entry(remaining_modules[0].clone())
    ...
    pub fn get_code(&self) -> Tokens {
        let sub_module_code = self.sub_modules.iter().map(|(name, module)| {
            ...
            quote!{
                ...
                pub mod #name {
                    use spatialos_gdk::worker::{EntityId, ComponentId};
                    use spatialos_gdk::worker::schema::{GeneratedSchema, Property, Event, Component,
                        ComponentDataInterface, ComponentUpdateInterface, Command,
                        CommandRequestInterface, CommandResponseInterface};
                    use spatialos_gdk::worker::ffi::{self, Schema_Object, Schema_ComponentData,
                        Schema_ComponentUpdate, Schema_CommandRequest, Schema_CommandResponse};
                    use schema::{Schema, ComponentData, ComponentUpdate};
                    use std::collections::HashMap;
                    use std::ops::Deref;
...
impl From<JsonCollection> for Global {
    fn from(value: JsonCollection) -> Global {
        let mut types: HashMap<String, Rc<RefCell<UserType>>> = value
            .typeDefinitions
            .into_iter()
            .map(|type_def| {
                (
                    type_def.qualifiedName.clone(),
                    Rc::new(RefCell::new(UserType::from(type_def))),
                )
            })
            .collect();

        let mut components: Vec<Component> = value
            .componentDefinitions
            .into_iter()
            .map(|component_def| Component::from(component_def))
            .collect();
    ...
    pub fn get_code(&self) -> Tokens {
        ...
        let command_request_deserialise = self.components.iter().flat_map(|component| {
            component
                .commands
                .iter()
                .map(|command| command.get_global_request_match())
        });
        let command_response_deserialise = self.components.iter().flat_map(|component| {
            component
                .commands
                .iter()
                .map(|command| command.get_global_response_match())
        });
        let unique_index_match = self.components
            .iter()
            .enumerate()
            .map(|(index, component)| {
        ...
        let update_enum_def = self.components.iter().map(|component| {
            let enum_name = Ident::new(component.enum_name().as_str());
            let data_qualified_name =
                Ident::new(format!("{}Update", component.rust_qualified_name));
            quote!(#enum_name(#data_qualified_name))
        });
        ...
        quote!{
            ...
            pub mod schema {
                use spatialos_gdk::worker::schema::{Component, GeneratedSchema,
                    GlobalComponentDataInterface, GlobalComponentUpdateInterface,
                    ComponentDataInterface, ComponentUpdateInterface,
                ...
                impl GeneratedSchema for Schema {
                    ...
                    fn serialise_entity_acl(read: Vec<String>, write: HashMap<ComponentId, String>)
                    ...
                    fn run_dynamic_component_handler<D: DynamicComponentHandler<Self>>
                        (handler: &mut D) {
                ...
                const GENERATED_NUMBER_OF_FIELDS: usize =
                    (1 + ((GENERATED_NUMBER_OF_COMPONENTS - 1) / spatialos_gdk::FIELD_SIZE_BITS));
                ...
                impl GlobalComponentDataInterface<Schema> for ComponentData {
                    fn deserialise(component_id: ComponentId, data: Box<Schema_ComponentData>)
                ...
                impl GlobalComponentUpdateInterface<Schema> for ComponentUpdate {
                    fn deserialise(component_id: ComponentId, update: Box<Schema_ComponentUpdate>)
